...
-- Removing document_role enum in favor of junction table approach
-- document_roles_assigned will be used to create many-to-many relationships between documents and roles
...
-- Skipping as type already exists
...
-- Constraint doesn't exist, skipping
...
drop index if exists "public"."idx_auth_clerk_orgs_members_clerk_organization_fkey";

drop index if exists "public"."idx_auth_user_clerk_orgs_members_user_id";

drop index if exists "public"."idx_auth_user_clerk_sessions_clerk_user_id";

-- Index doesn't exist, skipping
...
-- Skipping country_enum operations as it already exists
-- Comment out all country enum operations to avoid errors
...
-- Skipping drop of non-existent type
...
-- Skipping drop of type that has dependencies
...
-- Commenting out problematic column drops that may cause issues
...
-- Skip adding primary key constraint that would conflict with existing one
